-- DEFINEIX! · Enduriment de sessions, permisos i rendiment

revoke execute on function public.defineix_random_code(integer) from anon, authenticated, public;
revoke execute on function public.defineix_issue_device(uuid) from anon, authenticated, public;
revoke execute on function public.defineix_player_from_token(text) from anon, authenticated, public;
...
begin
  ...
  if found then
    ...
    left join public.daily_results dr
      on dr.player_id = p.id and dr.challenge_date = local_date
    where p.id = pid;
    return;
  end if;
  ...
  if attempt_total <> required_attempts then
    raise exception 'Nombre d’intents invàlid: se n’esperaven %', required_attempts;
  end if;

  select count(distinct value->>'wordId')::integer
    into distinct_words
  from jsonb_array_elements(p_attempts);
  if distinct_words <> attempt_total then
    raise exception 'No es permet repetir una paraula dins la mateixa partida';
  end if;

  if mode_value = 'daily' then
    select exists(
      select 1 from public.daily_results
      where player_id = pid and challenge_date = local_date
    ) into already_daily;

    if already_daily then
      return query
      select null::uuid, 0, 0, p.xp, p.points, p.daily_streak, p.daily_completed, dr.completed
      from public.players p
      join public.daily_results dr on dr.player_id = p.id and dr.challenge_date = local_date
      where p.id = pid;
      return;
    end if;
  end if;
  ...
  for item in select value from jsonb_array_elements(p_attempts) loop
    ...
    begin
      diff := (item->>'difficulty')::smallint;
    exception when others then
      raise exception 'Dificultat invàlida';
    end;
    if diff < 1 or diff > 5 then raise exception 'Dificultat invàlida'; end if;

    begin
      isperfect := (item->>'perfect')::boolean;
    exception when others then
      raise exception 'Resultat invàlid';
    end;
  ...
  set xp = p.xp + sum_xp,
      ...
      daily_streak = case
        when mode_value = 'daily' and daily_ok then
          case when p.last_daily_date = local_date - 1 then p.daily_streak + 1 else 1 end
        else p.daily_streak
      end,
      daily_completed = case
        when mode_value = 'daily' and daily_ok then p.daily_completed + 1
        else p.daily_completed
      end,
      last_daily_date = case
        when mode_value = 'daily' and daily_ok then local_date
  ...
  for s in select * from public.seasons
    ...
    values(
      ...
      perfect_total,
      case when mode_value = 'daily' and daily_ok then 1 else 0 end
    )
